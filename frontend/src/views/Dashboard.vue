<template>
  <div class="space-y-6 animate-fade-in">
    <!-- Page Header -->
    <div>
      <h1 class="text-3xl font-bold text-gray-900">{{ $t('dashboard.dashboard') }}</h1>
    ...
    <div v-if="loading" class="glass rounded-2xl shadow-lg p-12">
      ...
      <p class="text-center text-gray-500 mt-4">{{ $t('dashboard.loading') }}</p>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <router-link to="/patients" class="glass rounded-2xl shadow-lg p-6 hover:shadow-xl hover:scale-105 transition-all duration-200 cursor-pointer">
          <div class="flex items-center">
            ...
            <div class="ml-5">
              <p class="text-sm font-medium text-gray-500">{{ $t('dashboard.totalPatients') }}</p>
        ...
        <router-link to="/practitioners" class="glass rounded-2xl shadow-lg p-6 hover:shadow-xl hover:scale-105 transition-all duration-200 cursor-pointer">
          <div class="flex items-center">
            ...
            <div class="ml-5">
              <p class="text-sm font-medium text-gray-500">{{ $t('dashboard.totalPractitioners') }}</p>
        ...
        <router-link to="/appointments" class="glass rounded-2xl shadow-lg p-6 hover:shadow-xl hover:scale-105 transition-all duration-200 cursor-pointer">
          <div class="flex items-center">
            ...
            <div class="ml-5">
              <p class="text-sm font-medium text-gray-500">{{ $t('dashboard.totalAppointments') }}</p>
        ...
        <router-link to="/appointments" class="glass rounded-2xl shadow-lg p-6 hover:shadow-xl hover:scale-105 transition-all duration-200 cursor-pointer">
          <div class="flex items-center">
            ...
            <div class="ml-5">
              <p class="text-sm font-medium text-gray-500">{{ $t('dashboard.todayAppointments') }}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass rounded-2xl shadow-lg p-6">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('dashboard.appointmentsByStatus') }}</h3>
          <div class="h-64">
            <Doughnut v-if="appointmentsByStatusData" :data="appointmentsByStatusData" :options="chartOptions" />
        ...
        <div class="glass rounded-2xl shadow-lg p-6">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('dashboard.genderDistribution') }}</h3>
          <div class="h-64">
            <Pie v-if="genderDistributionData" :data="genderDistributionData" :options="chartOptions" />
            <div v-else class="flex items-center justify-center h-full text-gray-500">
              {{ $t('dashboard.noData') }}
            </div>
          </div>
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ $t('dashboard.appointmentsTimeline') }} - {{ $t('dashboard.last7Days') }}</h3>
        <div class="h-80">
          <Line v-if="appointmentsTimelineData" :data="appointmentsTimelineData" :options="lineChartOptions" />
          <div v-else class="flex items-center justify-center h-full text-gray-500">
            {{ $t('dashboard.noData') }}
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass rounded-2xl shadow-lg p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900">{{ $t('dashboard.recentPatients') }}</h3>
            <router-link to="/" class="text-sm font-medium text-primary-600 hover:text-primary-700">
              {{ $t('dashboard.viewAll') }} →
            </router-link>
          </div>
          <div v-if="recentPatients.length > 0" class="space-y-3">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-900 truncate">{{ getPatientName(patient) }}</p>
                <p class="text-xs text-gray-500">{{ patient.gender }}</p>
              </div>
            </div>
          </div>
          <div v-else class="text-center py-8 text-gray-500">{{ $t('dashboard.noData') }}</div>
        </div>

        <!-- Recent Appointments -->
        <div class="glass rounded-2xl shadow-lg p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-900">{{ $t('dashboard.recentAppointments') }}</h3>
            <router-link to="/appointments" class="text-sm font-medium text-primary-600 hover:text-primary-700">
              {{ $t('dashboard.viewAll') }} →
            </router-link>
          </div>
          <div v-if="recentAppointments.length > 0" class="space-y-3">
            ...
            >
              <div class="flex-1 min-w-0">
                <p class="text-sm font-medium text-gray-900 truncate">{{ getAppointmentPatientName(apt) }}</p>
                <p class="text-xs text-gray-500">{{ formatDateTime(apt.start) }}</p>
              </div>
              <span
