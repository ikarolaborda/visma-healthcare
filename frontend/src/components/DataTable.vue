<template>
  <div class="glass rounded-2xl shadow-lg overflow-hidden animate-fade-in">
              ...
              <div class="flex items-center gap-2">
                ...
                <span v-if="sortKey === column.key" class="text-primary-600">
                  ...
                  <svg v-else class="h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clip-rule="evenodd" fill-rule="evenodd" />
                  </svg>
                </span>
              </div>
            </th>
            <th v-if="$slots.actions" class="px-6 py-4 text-right text-xs font-semibold text-gray-700 uppercase tracking-wider">
              {{ $t('common.actions') }}
            </th>
          </tr>
        </thead>
          ...
          <tr v-if="paginatedData.length === 0">
            <td :colspan="columns.length + ($slots.actions ? 1 : 0)" class="px-6 py-12 text-center text-gray-500">
              <div class="flex flex-col items-center justify-center space-y-3">
                ...
                <p class="text-sm font-medium">{{ $t('table.noData') }}</p>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <!-- Pagination -->
    <div v-if="paginate && filteredData.length > 0" class="px-6 py-4 border-t border-gray-200 bg-gray-50/30">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
