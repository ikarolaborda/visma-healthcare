<template>
  <div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    >
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p class="mt-1 text-lg font-medium text-gray-900">
              {{ prescription.subject?.display || prescription.subject?.reference || 'N/A' }}
            </p>
          </div>
          <div>
            <p class="text-sm text-gray-500">
            ...
            <p class="mt-1 text-lg font-medium text-gray-900">
              {{ prescription.requester?.display || prescription.requester?.reference || 'N/A' }}
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- Dosage Information -->
      <div
        v-if="prescription.dosageInstruction && prescription.dosageInstruction.length > 0"
        class="glass rounded-2xl shadow-lg p-6"
      >
        <div class="border-b border-gray-200 pb-3 mb-6">
          <h3 class="text-lg font-semibold text-gray-900 flex items-center">
            <svg
        ...
        <div class="space-y-4">
          <div
            v-for="(dosage, index) in prescription.dosageInstruction"
            :key="index"
          >
            <p
              v-if="dosage.text"
            ...
            <div
              v-if="dosage.timing?.repeat?.boundsPeriod"
            ...
            >
              <div v-if="dosage.timing.repeat.boundsPeriod.start">
                ...
                <p class="mt-1 text-gray-900">
                  {{ formatDate(dosage.timing.repeat.boundsPeriod.start) }}
                </p>
              </div>
              <div v-if="dosage.timing.repeat.boundsPeriod.end">
                ...
                <p class="mt-1 text-gray-900">
                  {{ formatDate(dosage.timing.repeat.boundsPeriod.end) }}
                </p>
              </div>
            </div>
          </div>
      ...
      <!-- Additional Notes -->
      <div
        v-if="prescription.note && prescription.note.length > 0"
        class="glass rounded-2xl shadow-lg p-6"
      >
        <div class="border-b border-gray-200 pb-3 mb-6">
          <h3 class="text-lg font-semibold text-gray-900 flex items-center">
        ...
        <div class="space-y-3">
          <p
            v-for="(note, index) in prescription.note"
            :key="index"
            class="text-gray-700"
          >
            {{ note.text }}
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div v-if="prescription.meta?.lastUpdated">
            ...
            <p class="mt-1 text-gray-900">
              {{ formatDateTime(prescription.meta.lastUpdated) }}
            </p>
          </div>
        </div>
      </div>
