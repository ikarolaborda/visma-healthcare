  ...
  <div class="max-w-4xl mx-auto space-y-6 animate-fade-in">
    ...
    >
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div v-if="invoice.meta?.lastUpdated">
            ...
            <p class="mt-1 text-gray-900">
              {{ formatDateTime(invoice.meta.lastUpdated) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        ...
        <div>
          ...
          <p class="mt-1 text-lg font-medium text-gray-900">
            {{ invoice.subject?.display || invoice.subject?.reference || 'N/A' }}
          </p>
        </div>
      </div>
      ...
      <div class="glass rounded-2xl shadow-lg p-6">
        ...
        <div
          v-if="invoice.lineItem && invoice.lineItem.length > 0"
          class="space-y-3"
        >
          <div
          ...
          >
            <div>
              <p class="font-medium text-gray-900">
                {{ item.chargeItemCodeableConcept?.text || `Item ${index + 1}` }}
              </p>
            </div>
        ...
        <div class="border-t border-gray-200 mt-6 pt-4">
          <div class="flex justify-between items-center">
            <div>
              ...
              <p class="text-2xl font-bold text-gray-900">
                ${{ formatAmount(invoice.totalNet?.value || invoice.totalGross?.value) }}
              </p>
            </div>
          </div>
          <div
            v-if="invoice.totalGross && invoice.totalNet && invoice.totalGross.value !== invoice.totalNet.value"
          ...
          >
            ...
            <p class="text-lg font-semibold text-gray-900">
              ${{ formatAmount(invoice.totalGross.value) }}
            </p>
          </div>
        </div>
      </div>
      ...
      <!-- Payment Information -->
      <div
        v-if="invoice.totalPriceComponent && invoice.totalPriceComponent.length > 0"
      ...
      >
        ...
        <div class="space-y-3">
          <div
            v-for="(component, index) in invoice.totalPriceComponent"
            :key="index"
          ...
          >
            ...
            <p class="text-sm font-medium text-gray-900">
              ${{ formatAmount(component.amount?.value) }}
            </p>
          </div>
        </div>
      </div>
